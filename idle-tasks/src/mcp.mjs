...
import { context, repository, workdir } from './context.mjs';
...
export function serve() {
  ...
  // An agent gets the record only inside a repository: a session started in a scratch folder
  // must not reach every project's tasks. Our own install folder says nothing about where the work is.
  const inRepository = () => !!process.env.IDLE_PROJECT || (resolve(workdir()) !== OWN_FOLDER && repository() !== null);
  let hidden = false;
  ...
  async function call({ name, arguments: input = {} } = {}) {
    ...
    if (!inRepository()) return text(`refused: not inside a repository (${workdir()}) — the record of work is served only to sessions in a repository`, true);
    try {
  ...
  async function answer(message) {
    ...
    if (id === 'roots' && !method) {
      ...
      if (hidden && inRepository()) { hidden = false; send({ method: 'notifications/tools/list_changed' }); }
      return;
    }
    ...
    if (method === 'initialize') {
      ...
      send({ id, result: { protocolVersion: params?.protocolVersion ?? '2025-06-18', capabilities: { tools: { listChanged: true } }, serverInfo: { name: 'idle', version: '0.1.2' } } });
    } else if (method === 'ping') send({ id, result: {} });
    else if (method === 'tools/list') {
      hidden = !inRepository();
      send({ id, result: { tools: hidden ? [] : tools } });
    }
