...
- **The backend.** Tasks, decisions, claims, renders. Workflow-neutral: someone outside can use it with their own skills and their own way of working.
- **Our workflow.** The existing pipeline skills, adapted to read and write the backend instead of `.pipeline/` files. Not replaced.
...
4. **The human owns scope.** How far we go, what we need to reach, and roughly how we get there are decided by the human in discussion with the arbiter. The higher in the tree, the more the human is involved. There are no gates: an agent says done, the human contradicts and reopens. The arbiter's job is to surface enough for that ping-pong.
...
6. **Sessions are short; the substrate is the memory.** No agent, including the arbiter, is trusted to remember anything across sessions. What isn't written into the substrate doesn't exist.
...
10. **Conversation is the input, the substrate is the record.** Planning happens through interview with the arbiter. What persists is the curated result, not the transcript.
11. **One brain awake at a time.** A single arbiter plans every level of a stream. There are no sub-planners.
...
| checks | Verification commands, usually inherited from the environment's setup (its verify command) rather than written per task. We re-run them and own nothing about them — caching, baselines and flakiness are the verification system's business |
| scope | Strings naming what this task touches (paths, modules, systems). Written by the arbiter. The substrate refuses an active claim whose scope overlaps another active claim in the same stream |
...
| feedback | From the worker to the arbiter, capped: goal is wrong, approach won't work, conflicts with decision D-x, anything learned by doing |
...
| consumer_scenario | Root tasks: how the result is used from the outside, written by the arbiter at planning time, before implementation |
| metadata | Free-form: branch, worktree, container, URLs. Stored and rendered, never interpreted |

Conventions, not kinds:

- A **stream** is a root task, as big as the work needs. There are no tracks, work packages, or sizes; how a stream is divided — whether at all, how deep, how many agents — is an active decision each time. It ends when the arbiter marks it done; the human can reopen it until it is archived. Archived means merged or abandoned — both visible from the repo, which is more truth than the task system. Nothing is rendered back.
...
- A **proposal** is a set of child tasks created in `proposed` status by a worker who found the task bigger than it looked, or whose task *was* to work out the shape (architecture). The arbiter accepts, edits, or rejects. Work that does not belong to this stream is proposed as a new root; only the human accepts a root.
...
- **reopened**: the human contradicts a `done`, through the arbiter; the reason lands in `feedback` and the task returns to `ready`. Reopening a parent does not reopen its children; the arbiter decides what new work it means.
- **blocked**: the worker stopped and said why in `feedback`. The arbiter handles it on its next wake.
- A task with children is done when all children are done and its own goal is reached, via an integrate task the arbiter creates when the last child completes. Children being done is not the same as the goal being reached — that gap is what the integrate task and its review exist to catch.
- A **root task** is done when the arbiter judges its goal covered by done work and the consumer-view task has passed.

Stream liveness is found by walking the DAG from active root tasks: a live arbiter holds an unexpired claim on the root.

## 5. Verification

Uniform for every task; no per-task level. Three steps, in the order they were described in the Q&A (Parts 3 and 10):

- **Checks — re-run mechanically.** Workers say they ran the tests and didn't. So the worker never gets to say so: a routine re-runs the task's `checks` (and whatever the environment calls its verify command) and moves the task to `verified` or `rejected`. If a check needs setup, the setup is part of the command. This is a floor. Green checks do not mean the goal is reached.
...
The human decides what a stream is for and how far it goes. Under that, the arbiter creates tasks at every level; workers propose. A worker that finds its task too large returns it with proposed children rather than splitting in place. Creating a task or a decision shows the creator the nearest existing ones before it commits (search before create).

## 7. Roles

Roles are agent instruction files, not backend code.

**Arbiter.** Single per stream, and the agent the human talks to. Strongest available reasoning model, because it holds the whole picture. The human follows it: deep where it dives deep, shallow where it stays shallow. Two session types:
...
Three rules in the arbiter's instructions:
...
Steering always goes through the arbiter. Workers never see the interview; they see their task, the plan, and the decisions in scope.
...
- **Root state** for an arbiter wake and for the human's ping-pong: plan, Q&A, the tree with status, what was marked done since last wake and on what evidence, open proposals, blocked tasks with feedback, feedback on active tasks, decisions since last wake, carried-forward review notes.
...
| keepalive routine prompt | Expire stale claims, recompute ready, re-run checks for submitted tasks, create review / integrate / consumer-view / retro tasks, wake an arbiter if a stream has events and no live one |
...
| Skill | Becomes |
|---|---|
| `pipeline` | The arbiter's craft, in short sessions over the root-state render |
...
On this branch. Task, Decision, the `substrate` skill, minimal keepalive (expire claims, re-run checks, wake arbiter), one launch recipe; then adapt `pipeline`, `work-planning`, the interview skills and `write-code` to it. Re-run checks only, no review tasks yet. Run one real stream and find out where the model is wrong.
